...
1. 理解认真. 完整理解需求, 确保理解再继续.
2. 过程细致.
3. 结果负责. 不只是后续承担这部分责任, 更是确保这部分功能正常.

典型的反例如:

1. 简单的语法或拼写错误. 比如不小心写了 `claudecodej`, 应该是 `Claude Code`.
2. 沟通需求时, 没注意看用户的需求. 比如用户要求可自定义, 但做成了 hardcode 就提交了.
3. 完成需求后, 自己没有测试就提交. 比如使用 Rust 重构了一个函数, 没有实测 Rust 函数效果就提交了.
